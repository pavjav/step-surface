...
\begin{thm*}{Denote $\phi^{\theta}_{t}$ to be the flow in a vector direction $\theta$ to time t on the Necker cube.
Then the following is true:} \compat{There is no flow in a fixed direction because when trajectories move to new squares they may be traveling different directions.}
...
\item If $\theta$ is of the form (x,y) such that x or y are irrational, then $\phi^{\theta}_{t}$ is uniquely ergodic on $\mathbf{S}$. \compat{We should move the discussion of ergodicity outside the theorem.}	
\end{enumerate}
\end{thm*}
...
\\\cup  (\{m+1\}\times[n,n+1]\times[p-1,p]) 
\\\cup ([m,m+1]\times\{n+1\}\times[p-1,p])
\end{align*}
&
...
\noindent \compat{Inserted noindent}
is the family of these instances based at some integer triple. The surface will be comprised of the members that satisfy the property, $m+n+p=0$. From here on we will refer to the Necker cube surface's embedded form in $\mathbb{R}^{3}$ lying on the plane $z=-(x+y) $ as $\mathbf{S}$. That is to say that

$$\mathbf{S} =\bigcup\big{\{}\chi_{m,n,p}  : m+n+p=0 \big{\}}.$$
$\mathbf{S}$ now resembles the structure in Figure \ref{fig:cubexyz}. \compat{I would be more definitive. Say $S$ {\em is} the structure...}
...
\end{center}
\caption{$\mathbf{U}$ (left) and $\mathbf{U}'$ (right) differ only by translation. \compat{Perhaps just use one.}}
\label{fig:uprimeu}
\end{figure}

Now we will describe exactly how to construct $\mathbf{U}'$. Take the subset of the real plane, 
$$\mathbf{P} = \mathbb{R}^{2}-\bigcup_{m,n \in {\mathbb Z}} \{(u,v):2m-\frac{1}{2}<u<2m+\frac{1}{2}, 2n-\frac{1}{2}<v<2n+\frac{1}{2} \},$$ 
