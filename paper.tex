...
A geodesic as viewed as line segments on $\mathbf{P}$ can travel in one of four possible directions at any given time. On $\mathbf{U}$ a geodesic flow's direction is characterized as an element of the set of all possible directions as equivalence classes of $\mathbb{R}/2\pi\mathbb{Z}$ of the form $\mathbf{V}_{\theta}=\{\theta,\theta+\frac{\pi}{2},\theta+\pi, \theta+\frac{3\pi}{2} : \theta\in[0,\frac{\pi}{2})\}$. To each member of the class we associate an element of $\mathbb{Z}/4\mathbb{Z}$ to signify the current phase of the geodesic flow. We use this construction to create our four-fold cover of $\mathbf{U}$ as a topological quotient of $\mathbf{P}\times\mathbb{Z}/4\mathbb{Z}$. For every even integer pair centered around every hole of $\mathbf{P}\times\mathbb{Z}/4\mathbb{Z}$, assign a pair of integers (m,n). Edge identifications are made on the boundaries of every hole taking one phase plane to the next in the following manner:

\begin{center}
\includesvg[width=4in]{utilda}
\end{center}
This is a branched cover of $\mathbf{U}$ of degree four,$\tilde{\mathbf{U}}_0$, with the obvious covering map taking every non-singular point and projecting it onto $\mathbf{U}$. We define the edge identifications as a relation on $\mathbf{P}\times\mathbb{Z}/4\mathbb{Z}$. For $i\in[-\frac{1}{4},\frac{1}{4}]$:


\begin{multline}
([2m+t], 2n-\frac{1}{4},j)\sim(2m-\frac{1}{4},[2n+t],j+1)\\
([2m+t], 2n+\frac{1}{4},j)\sim(2m+\frac{1}{4},[2n+t],j+1)\\
\vspace{0.2in}For (m,n;j) \in \mathbb{Z}^{2}\times\mathbb{Z}/4\mathbb{Z}
\label{eq:rel2}
\end{multline}

We define $\tilde{\mathbf{U}}_0$ to be the topological quotient, $(\mathbf{P}\times\mathbb{Z}/4\mathbb{Z})/\sim$. $\tilde{\mathbf{U}}_0$ is a Euclidean cone surface with four distinct vertices for each hole centered at $(m,n;\mathbb{Z}/4\mathbb{Z})$ on  $\mathbf{P}\times\mathbb{Z}/4\mathbb{Z}$ of cone angle $6\pi$ with proper identification. Now we will show that $\tilde{\mathbf{U}}_0$ is isometric to a translation surface via an induced isometry on $\mathbf{P}\times\mathbb{Z}/4\mathbb{Z}$.
...
\begin{Def} Let $\sim$ be the equivalence relation $\eqref{eq:rel2}$, and $\phi$ be the isomorphism on $\mathbf{P}\times\mathbb{Z}/4\mathbb{Z}$ $\eqref{eq:phi}$. Define a new relation obtained from $\sim$ as:  $x_1\sim_{\phi}x_2$ if $\phi^{-1}(x_1) = \phi^{-1}(x_2)$ for $x_1,x_2\in\mathbf{P}\times\mathbb{Z}/4\mathbb{Z}$. We define $\tilde{\mathbf{U}}$ to be  $(\mathbf{P}\times\mathbb{Z}/4\mathbb{Z})/\sim_{\phi}$.
\end{Def}
...
$\phi$ preserves the edge identifications on $\tilde{\mathbf{U}}_0$ by affine transformations. And since $\tilde{\mathbf{U}}_0$ is a cover of $\mathbf{U}$, $\tilde{\mathbf{U}}$ is a translation cover of $\mathbf{U}$. Observe that by rotating each phase plane, the manner in which the holes are indexed undergoes the same transformations from our perspective.
...
Alternatively, the edge identifications made on $\mathbf{U}$ that gave us the set $\mathbf{V}_\theta$ let us define a flow $F^{t}:T_1(\mathbf{U})\rightarrow T_1(\mathbf{U})$ as $F^{t}(p,v)=(p+tv,v)$, where $(p,v)$ is a member of the unit tangent bundle on point $p$ of $\mathbf{U}$ in direction $v$ of $\mathbf{V}_\theta$. Defining a map from $d:T_1(\mathbf{U})\rightarrow \mathbf{V}_\theta$ that returns the direction $v$. $\tilde{\mathbf{U}}$ can be abstractly realized as the preimage of $d$, $d^{-1}(\mathbf{V}_\theta)$.
...
Observe that $\tilde{\mathbf{M}}$ can be reconstructed by a series of cutting and gluing operations and realized as a square tiled staircase:

\begin{figure}[H]
\centering
\includesvg[width=4in]{mtildastaircase}
\caption{$\tilde{\mathbf{M}}$ as a staircase with vertices $x_i$ and phases identified.}
\end{figure}

Every edge pair on the staircase model has an exact distance of two. (Each square is unit). This gives way to a very natural cylindrical decomposition.
...
The staircase is then decomposed into 6 uniform cylinders of moduli $\frac{1}{2}$ in both the vertical and horizontal directions. All families of saddle connections on the surface intersect exactly two others.

\begin{Def}
The \textbf{family of horizontal cylinders} of $\tilde{\mathbf{M}}$, denoted $\Delta_{i}$ where $1\leq i \leq 6$, are cylinders formed by saddle connections made between singularities $((x_3,x_2)\cup(x_2,x_3)$, $(x_3,x_4))$ when i is odd, and $((x_1,x_4)\cup(x_2,x_4)$, $(x_1,x_2))$ when i is even. \newline
Likewise, the \textbf{family of vertical cylinders}, denoted $\varepsilon_j$ $(1\leq j 
\leq 6)$, are formed by connections $((x_2,x_3)$, $(x_2,x_1)\cup(x_1,x_2))$ when j is odd, and  $((x_4,x_1)$, $(x_4,x_3)\cup(x_3,x_4))$ when j is even.
\end{Def}
...
A major benefit of $\tilde{\mathbf{M}}$ being tiled by unit squares is that the surface has all of the same symmetries of the square. The action of $\mathbb{D}_4$ on $\tilde{\mathbf{M}}$ preserves its boundaries up to translation.
