...
\end{wrapfigure}
\noindent Let $x_0$ be a point on the surface that is contained in the interior of a face and consider a tangent unit vector $v\in\RR^3$ based at a point $u_0$. Each face is parallel to a $2$-dimensional subspace of $\RR^3$ spanned by exactly two basis vectors, so one component of $v$ is 0. Call this projected vector $v_0\in\RR^2$.
\\\\
The angle $v_0$ makes relative to a choice of basis is the \emph{initial trajectory angle} of the unit-speed geodesic on the surface which we denote by $\Phi_t:\RR\rightarrow\bUs$. A rational initial trajectory in $\RR^2$ falls into one of two categories:
\begin{Def}
...
\end{Def}
\noindent This paper aims to depict the relationship between initial trajectory angles and dynamical properties of a geodesic on the surface. Denote $\bUs$'s translational symmetries by $\text{Trans}(\bUs)$. Recall that a geodesic on the surface is \emph{periodic} if there exists some $T>0$ such that $\Phi(t+T)=\Phi(t)$ for all $t\in\RR$ and a geodesic is \emph{drift-periodic} if there exists some $T>0$ and non-trivial $p\in \text{Trans}(\bUs)$ such that $\Phi(t+T)=p(\Phi(t))$ for all $t\in\RR$. In either case we call $T$ the \emph{period} of $\Phi$ if it is the \emph{smallest} possible value that satisfies these criteria. We additionally require that at no point does $\Phi$ encounter a cone singularity of the surface, for which geodesic behavior is non-deterministic.


\begin{thm*}
(Dynamical Properties of a Geodesic on $\bUs$) Let $\Phi_t:\RR\rightarrow\bUs$ be a non-singular unit-speed geodesic on the Necker cube surface such that $\Phi(0)=u_0$ and $\Phi'(0)=v v_0\in\RR^2$. Then the following is true:
\begin{enumerate}[label=(\roman*)]
\item $\Phi$ is periodic with period $T\in\RR$ if  $v_0\in\cO$.
\item $\Phi$ is drift-periodic with period $T\in\RR$ if $v_0\in\cE$.
\end{enumerate}
\end{thm*}
...
\begin{figure}[H]
\centering
\includesvg[width=1.6in.]{neckercube}\includesvg[width=2in.]{neckercubepath}
\end{figure}
...
\subsection{Deck Transformations of the $\ZZ^2$ Cover}
The set of Deck transformations of the covering map $\bUs\twoheadrightarrow\bGs$ is isomorphic to the deck group $\ZZ^2$ that we induce from $\text{Trans}(\CC)$. First we remove open unit squares on an even integer lattice in the plane:\\

\noindent  Let $m,n\in\ZZ$.
\begin{equation}
\label{eq:P}
...
\noindent Now make the following identifications on $\mathbf{P}:$
\\ Let $\aleph_{m,n}=2m+i2n\in\CC$ be the center of a removed square. Now let $\zeta_{m,n}(t)=(2m+t)+i(2n-\frac{1}{2})$ and 
$\xi_{m,n}(t)=(2m+t)+i(2n+\frac{1}{2})$ be families of paths on $\mathbf{P}$ that run along the edges of the squares, parameterized by $t\in (\frac{-1}{2},\frac{1}{2})$. $\bUs$ is recovered as a topological quotient by making the following edge identifications:

\begin{equation}
\begin{split}
(\zeta(t))\sim(e^{i\frac{\pi}{2}}~\overline{\zeta(t)-\aleph})\\
(\eta(t))\sim_(e^{i\frac{\pi}{2}}~\overline{\eta(t)-\aleph})
\label{eq:rel2}
\end{split}
\end{equation}

These spaces and identifications could be adapted to recover $\bGs$ by restricting $\mathbf{P}$ to a $2\times2$ square about the origin, only allowing $m=n=0$, and identifying the outer edges of the square much like a 2-torus. The translations $+2,+2i\in \text{Trans}(\CC)$ induce horizontal and vertical maps $f_h,f_v\in\text{Trans}(\bUs)$. The group $\<f_h,f_v\>$ is isomorphic to $\ZZ^2$.
...
The key connection made between surfaces $\bUs$ and $\btUs$ is that geodesics on $\bUs$ and $\btUs$ are identical in the sense that their images are in a one-to-one correspondence. Unless stated otherwise, a \emph{geodesic} is a \emph{unit-speed} locally distance minimizing curve on a smooth topological surface as a function parameterized by an element in $\RR$. A \emph{geodesic segment} is a restriction of the geodesic to some closed interval in $\RR$. A geodesic has an initial trajectory defined at 0, represented by a unit vector $v_0\in\RR^2$. If the vector can be scaled by $k\in\RR$ such that $kv_0\in\cO\cup\cE$, the geodesic has a \emph{rational} initial trajectory. For example, the following is a periodic geodesic on the surface with initial direction $kv_0\in\cO$:
...
The geodesic segment on $\bGs$ has the property that it returns to $x_0$ with a trivial effect on the holonomy of the curve. On the other hand, the lifted paths will close only if their homotopy classes belong in their fundamental groups, i.e. the kernel of homomorphisms $\varphi_1$, $\varphi_2|_{\ker\varphi_1}$. Otherwise they are mapped non-trivially to $\ZZ^2$ or $\ZZ^2\times\SO(2,\ZZ)$. Let $v_0\in\RR^2$ be a unit vector such that $\alpha's$ image under the derivative map, $\alpha'$, is $v_0$ at times $0$ and $b$. The derivatives of the lifts $\gamma', \gamtild'$ are also $v_0$ at their endpoints.
...
On the Necker cube surface, a horizontal translation, $f_h$, is induced by $2\in\text{Trans}(\CC)$, and a vertical translation, $f_v$, is induced by $2i\in\text{Trans}(\CC)$. The group of deck transformations of the surface given by the cover $\bUs\rightarrow\bGs$ is isomorphic to the group $\Delta_{\varphi_1}=\pi_1(\bGs)/\pi_(\bUs)\cong\ZZ^2$. Take $\ZZ^2$ to the deck group by sending $(1,0)$ to $f_h$ and $(0,1)$ to $f_v$. Similarly, $\btUs$ has deck transformations given by the cover $\btUs\rightarrow\bGs$ induced by elements in $\text{Trans}(\CC\times\SO(2,\ZZ))$. These are isomorphic to the deck group given by the cover $\btUs\rightarrow\bUs\rightarrow\bGs$ containing elements of the form $((m,n),M)\in\ZZ^2\rtimes_{\eta}\SO(2,\ZZ)$, where $\eta:\SO(2,\ZZ)\rightarrow \Aut(\ZZ^2)$. $\SO(2,\ZZ)$ already belongs in $\Aut(\ZZ^2)$ so $\eta$ is an inclusion endomorphism, and the binary operation of this group is defined as  $((m,n),M)\cdot((m',n'),M')=((m,n)+M(m',n'),MM')$.\\
\\
Let $\varphi:\pi_1(\bGs)\rightarrow \ZZ^2\rtimes_{\eta}\SO(2,\ZZ)$ be given by $[g]\mapsto(\varphi_1([g]),\varphi_2([g]))$. $\varphi$ is a

\begin{thm}
Let $\delta=\varphi_1([\alpha])\in\ZZ^2$ be an element of $\Delta_{\varphi_2}$ and $\tilde{\delta}=\varphi([\alpha])\in\ZZ^2\rtimes_{\eta}\SO(2,\ZZ)$ be an element of $\Delta_{\varphi_{2}\vert_{\pi_1(\bUs)}}$. Then:\\
...
\begin{lem}
Let $v_0\RR^2$. Then for any geodesic $\alpha$
\end{lem}
