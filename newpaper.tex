...
This section details the construction of the Necker cube surface and how two different interpretations of the surface are one and the same.
\subsection{Geometric Interpretation}
Consider the following surface embedded in $\RR^3$:
\begin{figure}[H]
\centering
\includesvg[width=2in.]{neckercube}
\end{figure}

We call this surface $\bG$, the \emph{Necker cube}. $\bG$ is obtained by taking three copies of the unit square and identifying the edges so as to resemble a cube when viewed from the front. $\bG$ has a total of three vertices. Every vertex has a cone angle of either $3\pi$ or $\frac{3\pi}{2}$ and is therefore a conical point/singularity. It is apparent that $\bG$ has an induced flat metric and non-trivial linear holonomy, so geodesic flows on its unit tangent bundle are well-defined via parallel transports over the edges. We denote the surface without singularities by $\bGs$. The periodic tiling of $\bG$ will take infinitely many copies of $\bG$ and glue them translationally along the solid triangles and create our Necker cube surface, denoted $\mathbf{S}$.\\
...
\compav{Redo this maybe define the cover by monodromy map}
