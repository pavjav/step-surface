...
We denote the \emph{unit tangent bundle} on the surface $\bUs$ by $T^1\bUs$, and construct a cover with trivial holonomy. Initial experiments have shown us that geodesics viewed as discontinuous line segments on $\mathbf{P}$ flow in any one of four directions at a time.
\begin{figure}[H]
\centering
...
\end{figure}
Observe that a closed loop on $\bUs$ will act on vectors tangent to the surface by a rotation of $\pm\frac{\pi}{2}$ radians (since the surface is flat and embedded in $\CC$, we work with vectors in $\RR^2$).
...
Let $\omega$ be a flat, Riemannian metric connection on $T^1\bUs$. This is allowable since $\bUs$ is obtained as the cover of a flat Riemann surface. We say that $p=(x_0,v_0)\in T^1\bUs$ is a fixed point in the fiber over $x_0$ in the unit tangent bundle of the surface, with $x_0\in\bUs$ and $v_0\in\RR^2$ being unit. Denote the \emph{holonomy group} of the connection with base point p by $Hol_p(\omega)=\{A\in SO(2,\RR)~:~ p \equiv A\cdot p\}$ where $p$ differs from $A\cdot p$ by some rotation of $v_0$, and the \emph{holonomy bundle}  by $H(p)$, the set of points $p'\in T^1\bUs$ where a closed loop on $\gamma:[0,1]\rightarrow\bUs$ lifts to $\gamma^*:[0,1]\rightarrow T^1\bUs$ such that $p=\gamma^*(0)$ and $p'=\gamma^*(1)$.
...
Observe that when $r$ acts on the relations defined on $\ref{eq:rel2}$ we get a new relation:
...
We recall some definitions and properties of translation surfaces and their translation covers. A \emph{translation surface}, $S$, is a compact, flat surface that admits a discrete number of conical singularities and has trivial linear holonomy on arbitrary paths. We denote the surface without singularities by $S^\circ$. Translation surfaces belong to a stratum or family of surfaces characterized by the number of conical points and their degrees $(\text{cone angle}-2\pi)$. A $Z^2$-cover is a cover of $S$ given by a map $f:\tilde{S}\rightarrow S$, such that the associated deck group of the cover, $\Delta=\pi_1(S)/\pi_1(\tilde{S})=\pi_1(S^\circ)/\pi_1(\tilde{S}^\circ)$, is isomorphic to $\ZZ^2$.

\begin{Def}
Intersection number is a non-degenerate bilinear form:
\begin{align*}
i:H_1(S,\mathbf{R})\times H_1(S,\mathbf{R})\rightarrow\mathbf{R},
\end{align*}
where $\mathbf{R}$ is abelian and for $[\gamma],[\beta]\in H_1(S,\mathbf{R})$, $i([\beta],[\gamma])$ returns the intersection number as an element of $\mathbf{R}$, where a positive crossing has $[\gamma]$ make a positive angle relative to $[\beta]$.
\end{Def}

\begin{Def}
Let $u,v\in H_1(S,\mathbf{R})$ be homology classes of curves on $S$ that span $H_1(\tilde{S},\mathbf{R})$. Then there is a group homomorphism from $\pi_1(S)$ to $\mathbf{R}^2$ defined as:
\begin{align*}
...
\end{align*}
Since $\alpha$ is non-singular, it can be mapped to $S^\circ$ which admits a flat holomorphic one-form $dz$. Let $\theta=Arg(\mathbf{hol}([\alpha]))$. We say that $\phi_t^\theta:[0,\infty)\rightarrow S^\circ$ is the unit-speed geodesic flow on $S^\circ$ given by $[\alpha]$ such that $\phi^\theta_0=\alpha(0)$.
\end{Def}
...
Since $\pi_1(\bM)\leq\pi_1(\bM^\circ)$, we may work with the absolute homology of $\bM$ to prove results on $M^\circ$. Consider the following cylinder curves on $\bM$:
...
\begin{Def}
The set of abelianized cylinder core curves is $\Gamma=\{\gamma_i~: ~0\leq i\leq11\}\subset H_1(\bM,\ZZ)\subset H_1(\mathbf{M},\mathbb{Q})$.
\end{Def}

It can be shown that $\Gamma$ spans $H_1(\bM,\QQ)$ by constructing a $12\times12$ matrix $M$ such that $M_{j,k}=i(\gamma_k,\gamma_j)$. Every row has two entries that alternate between $1$ when $j$ is odd and $-1$ when $j$ is even. Such a matrix is rank 10, precisely the dimension needed for a genus 5 surface. 
